...
public class ProxyConnection extends NetClient {
 ...
 if (Base64.getEncoder().encodeToString(faviconBytes).isBlank()) {
 		...
 		if (ViaProxy.getConfig().getCustomMotd().isBlank()) {
            ...
            future = this.c2p.writeAndFlush(new S2CStatusResponsePacket("{"+favicon+CustomMotd+"\"players\":{\"max\":10,\"online\":-9999},\"version\":{\"protocol\":774,\"name\":\"Paper 1.7.2-26.1.2 :3\"}}"));
